...
## 2026-09-21 — 신뢰도 가중 fusion (단순 평균 → frontality 가중)

**배경:** 2026-09-15 고정타겟 파일럿에서 단독 카메라 43.03° vs fusion 43.47° — **fusion이
아무 이득도 못 냈다.** 원인은 좋은 추정과 나쁜 추정을 같은 무게로 평균했기 때문이고, 같은
파일럿에서 오차가 "얼굴이 그 카메라에서 얼마나 벗어나 보이는가"를 그대로 따라간다는 것도
확인됐다(편위 22~39°에서 7~28°, 87~89°에서 38~62° — `results_summary.md`). 즉 **어느 카메라를
덜 믿어야 하는지는 추정 결과를 보기 전에 미리 알 수 있다.** multi-view가 실제로 이득을 내려면
이 정보를 써야 한다.

**구현:**

1. `gaze_bridge_node.py` — `/{camera}/face_frontality_deg`(Float32) 신규 발행.
   얼굴 전방축(MediaPipe `facial_transformation_matrixes`, 이미 받아놓고 fallback 외에는
   안 쓰던 값)과 **머리→카메라 방향** 사이 각도. 0°=카메라를 똑바로 마주봄.
   - 광축(−z)이 아니라 머리→카메라 방향을 기준으로 삼는다: 파일럿에서 머리가 광축에서
     ~0.1m 벗어나 앉아 약 11° 차이가 났고, `results_summary.md`도 그 근사를 한계로 적어뒀다.
     머리 위치를 쓰면 그 오차가 사라진다.
   - **전방축 부호 자동 판별**: `-z` 전방 가정(기존 `get_gaze_vector_from_matrix()`가 쓰던
     것)은 이 파이프라인에서 실측 검증된 적이 없다(fallback 경로라 거의 안 쓰임). MediaPipe는
     뒤통수를 얼굴로 검출하지 못하므로 실제 각도는 항상 90° 미만이어야 한다 — 초기 30프레임
     cos 중앙값이 음수면 부호가 반대라는 뜻이므로 경고 후 자동 보정한다.

2. `gaze_fusion_node.py` — 가중치 = `cos(frontality)`, 하한 `MIN_WEIGHT=0.01`.
   - 일치 구간: head_position/gaze_direction을 **가중평균**(기존엔 단순 평균).
   - 불일치 fallback 기준 변경: 기존 "더 최근에 갱신된 카메라"는 신뢰도 신호가 없어서 쓰던
     임시 대체물이었다(2026-08-25 주석에 그렇게 적혀 있음). 이제 **"얼굴을 더 정면으로 보는
     카메라"**를 고른다 — 오차가 편위각을 따라간다는 결과에 직접 맞는 기준.
   - **하위호환**: frontality가 안 들어오거나 stale하면 조용히 기존 균등 평균으로 되돌아간다
     (WARN 1회). 구버전 bridge와 섞어 띄워도 안 깨진다.

**효과 예시**(파일럿 기하 그대로): 22.6° 카메라 w=0.923 vs 87.2° 카메라 w=0.049 — **약 19배**.
즉 준측면 카메라의 나쁜 추정이 사실상 배제된다.

**검증:** 카메라 없이 도는 스크립트 2개 추가 (`scripts/tests/`, 실행법은 그쪽 README).
frontality 각도 계산/부호 판별 13개, 융합 시나리오 8개 전부 통과. 작성 중 실제 버그도
하나 잡음(frontality 없는 경로에서 로그 포맷이 `None`을 만나 콜백이 죽던 문제).

**아직 안 한 것 (실카메라 필요):**
- [ ] 전방축 부호 자동 판별이 실제 MediaPipe 출력에서 어느 쪽으로 결론 내는지 확인
      (노드 시작 후 로그 1줄로 나옴). 얼굴을 좌우로 돌리면서 `/kinect1/face_frontality_deg`가
      단조증가하는지도 같이 볼 것 — 이게 이번 변경의 유일한 미검증 가정이다.
- [ ] `eval_gaze_accuracy.py`가 frontality도 같이 녹화/집계하게 해서 **오차-편위각 곡선**을
      그릴 것. 지금은 타겟 3개라 점 3개뿐 — 타겟 배치 재설계(아래)와 같이 가야 의미가 생긴다.
- [ ] 평가 프로토콜 재설계: 현재 타겟 3개가 거리 0.33~0.93m로 제각각이고 2개가 ±88° 준측면
      (HRI에서 사람을 관찰하는 영역이 아님). 1.5~2.5m 등거리 9점 그리드로 바꿔야 가중 fusion의
      효과를 측정할 수 있다.
- [ ] Kinect intrinsics 실측 캘리브레이션 (`fx=fy=525` 근사값이 head_position과 평가
      기준선에 동시에 들어가 있어 지금 오차의 분해를 막고 있음).
